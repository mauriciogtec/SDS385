...
saveRDS(resultsPvalue, file = "myCache/benchmarkPvalue.RDS")
```

```{r, include = FALSE}
resultsPvalue <- readRDS("myCache/benchmarkPvalue.RDS")
for (i in 1:length(resultsPvalue)) {
  assign(names(resultsPvalue)[i], resultsPvalue[[i]])
}
```

```{r}
data_frame(
  P = scales::comma(Pvalue),
  QR = paste0(qr_time, "s"),
