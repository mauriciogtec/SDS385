...
$$
\beta = \arg\min_{\beta\in\mathbb{R}^N} \sum_{i=1}^N w_i(y_i - x_i^\top\beta)^2,
$$
...
#### Part II of solution: The QR factorization for WLS
...
### (C) Coding and testing the method

***Code up functions that implement both the inversion method and your method for an arbitrary $X$, $y$, and weights $W$.Now simulate some silly data from the linear model for a range of values of $N$ and $$. (Feel free to assume that the weights wi are all 1.) It doesn’t matter how you do this—e.g. everything can be Gaussian if you want. (We’re not concerned with statistical principles yet, just with algorithms, and using least squares is a pretty terrible idea for enormous linear models, anyway.) Just make sure that you explore values of P up into the thousands, and that $N$ > $P$. Benchmark the performance of the inversion solver and your solver across a range of scenarios.***
...
```{r, eval = FALSE}
time_grid_N <- data.frame(
  Nvalue = floor(10^seq(4, 6, by = 0.5)) # 10, 31, 100, 316, etc..
)
time_grid_N$qr_time <- 0
time_grid_N$inv_time <- 0
for (i in 1:nrow(time_grid_N)) {
  n <- Nvalue[i]
  t_qr <- microbenchmark(wls_qrfit(X[1:n, 1:P], y[1:n], weights[1:n]), times = 5L)
  t_inv <- microbenchmark(wls_invfit(X[1:n, 1:P], y[1:n], weights[1:n]), times = 5L)
  time_grid_N$qr_time[i] <- mean(t_qr$time)  / 1e9
  time_grid_N$inv_time[i] <- mean(t_inv$time)  / 1e9
}
```
...
saveRDS(time_grid_N, file = "myCache/benchmarkNvalue.RDS")
```

```{r, include = FALSE}
time_grid_N <- readRDS("myCache/benchmarkNvalue.RDS")
```
...
```{r}
plot_data <- time_grid_N %>% 
  mutate(logN = log(Nvalue, 10)) %>% 
  select(-Nvalue) %>% 
...
These results are consistent with our theory that said if $N$ is similar to $P$ then computation time should be efficient for the $QR$ method, but as $N >> P$, the QR algorithm needs twice as much work because it requires the factorization of $X$.
...
```{r, eval = FALSE}
time_grid_P <- data.frame(
  Pvalue = floor(10^seq(2, 4, length.out = 11)) - 1 # 100, 316, 1000, etc..
)
time_grid_P$qr_time <- 0
time_grid_P$inv_time <- 0
for (i in seq_along(Pvalue)) {
  ...
  time_grid_P$qr_time[i] <- mean(t_qr$time)  / 1e9
  time_grid_P$inv_time[i] <- mean(t_inv$time)  / 1e9
}
```
...
```{r, eval = FALSE, include = FALSE}
saveRDS(time_grid_P, file = "myCache/benchmarkPvalue.RDS")
```

```{r, include = FALSE}
time_grid_P <- readRDS("myCache/benchmarkPvalue.RDS")
```
...
```{r}
plot_data <- time_grid_P %>%
  mutate(logP = log(Pvalue, 10)) %>% 
  select(-Pvalue) %>% 
  gather(method, seconds, -logP)
ggplot(plot_data, aes(x = logP, y = seconds, colour = method)) +
  geom_line(size = 1) +
  scale_colour_discrete(labels = c("Full Inverse", "QR")) + 
  ggtitle("Effect of increasing P with N = 10,000 fixed") + 
  theme_calc() 
```

From this last plot it seems pretty obvious that although the QR method has many benefits in numerical stability, it does not scale very well. The increase in $P$ affects the QR method even more than an increase in $N$.


### (D) The Sparse World


***Now what happens if X is a highly sparse matrix, in the sense that most entries are zero? Ideally we’d realize some savings by not doing a whole bunch of needless multiplication by zero in our code. ***
...
In R, the library `Matrix` handles sparse matrices, having a special S4 class. For the uninitiated is R's formal S4 classes, the thing to remember is that implication of having a class is that multiplication, summation, and many function can automatically identify if the input matrix is of this class, and then call appropriate corresponding algorithm. La `Matrix` not only handle sparse matrices, but in general optimal algorithms for several clases such as diagonal, triangular, dense, sparse, sparse diagonal, sparse triagular, etc. The programming concept of inheritence allows one objecto to have more than one class.

```{r}
library(Matrix)
```

#### A QR algorithm for sparse data

It turns out that we don't have to use any modifications to our code, this is because the way the Matrix package is designed, all the functions we used are overloaded to call specialezed routines for sparse data if the input Matrix has the class sparse. I will break this down.
...
We see this object has a class `dgCMatrix`, which is just a code for sparse matrix. The `dcGMatrix` class is a subclass of `Matrix`, so our first line of code reads

```{r}
inherits(X, c("matrix", "Matrix")) # X will inherit from "Matrix"
```
Now the next part, we shall see that even after the multiplications with the weights, `Xtilde` is sparse
...
The QR factorization routine called when the inpute is sparse is a special method for the input. The output has a tag `sparseQR`
...
#### Benchmarking with sparse data

We will now use the  Matrix package and simulated large sparse data and repeat our experiments of the previous answer.


##### The effect of sample size $N$ with different values of sparsity
...
*Note:* you could use the function `objetct.size` and vertify that the storage size of `X` greatly reduces once declared as sparse
...
set.seed(110104) # reproducibility
Xdense <- rnorm(N * P) %>% 
  matrix(ncol = P) # We will mask Xdense later in our code with the different levels
# random true model
beta <- rnorm(P) # random coefficients
sigma <- .01 # error size
y <- drop(X %*% beta) + rnorm(N, sd = sigma)
...
Now we run the same simulations

```{r, eval = FALSE}
# Values to test
Nvalue <- floor(10^seq(4, 6, by = 0.5)) # 100, 316, 1000, etc..
sparsity <- c(.05, .25, .50, .75, 1) # used to create many zeros later in X later in the code
# create a grid of combinations of sparsity and N, and an instructions if whether use the sparse verion
time_grid_N_sparse <- expand.grid(
  sparse_rate = sparsity, 
  Nvalue = Nvalue, 
  use_sparse = c(TRUE, FALSE)
)
time_grid_N_sparse$qr_time_sparse <- 0
time_grid_N_sparse$inv_time_sparse <- 0
for (i in 1:nrow(time_grid_N_sparse)) {
  n <- time_grid_N_sparse$Nvalue[i]
  s <- time_grid_N_sparse$sparse_rate[i]
  use_sparse <- time_grid_N_sparse$use_sparse[i]
  mask <- rbinom(n * P, 1, s)
  # this line will create assign a class to X that will determine if uses sparse algorithms or not
  X <- (Xdense[1:n, 1:P] * mask)
  if (use_sparse) {
    X <- Matrix(X, sparse = use_sparse)
  }
  t_qr <- microbenchmark(wls_qrfit(X, y[1:n], weights[1:n]), times = 5L)
  t_inv <- microbenchmark(wls_invfit(X, y[1:n], weights[1:n]), times = 5L)
  time_grid_N_sparse$qr_time_sparse[i] <- mean(t_qr$time)  / 1e9
  time_grid_N_sparse$inv_time_sparse[i] <- mean(t_inv$time)  / 1e9
}
```
...
```{r, eval = FALSE, include = FALSE}
saveRDS(time_grid_N_sparse, file = "myCache/benchmarkNvalueSparse.RDS")
```

```{r, include = FALSE}
sparsity <- c(.05, .25, .50, .75, 1)
time_grid_N_sparse <- readRDS("myCache/benchmarkNvalueSparse.RDS")
```

```{r}
time_grid_N_sparse %>% 
  knitr::kable(align = "rrrr") # for nice table format in rmarkdown
```

We now plot the results; as expected, non-sparse method behave the same independently of the sparsity level and the sparse methods behave badly if the matrix is not sparse. Surprisingly, however, the sparse methods are overall slower than the non-sparse methods. This can only mean that the `Matrix` package routines for sparse matrices not very well optimized for the sparse case, and perhaps we should pick other algorithms.

*Note about performance*: I am running the Microsoft Open R version of R, which heavily optimized and parallelized for `matrix` objects. I am also running my code on a 8 core computer, which benefits from the improvements of the Microsoft version of R. This might be the reason for the performance difference.

```{r}
plot_data <- time_grid_N_sparse %>% 
  mutate(logN = log(Nvalue, 10)) %>% 
  select(-Nvalue) %>% 
  mutate(sparse_rate = ordered(time_grid_N_sparse$sparse_rate, levels = sparsity)) %>% 
  gather(method, seconds, -c(logN, sparse_rate, use_sparse)) 
ggplot(plot_data) +
  geom_line(aes(x = logN, y = seconds, colour = method, linetype = use_sparse), size = 1) +
  facet_grid(sparse_rate ~ .) +
  scale_colour_discrete(labels = c("Full Inverse", "QR")) +
  scale_linetype_discrete(labels = c("No", "Yes")) +
  ggtitle("Effect of increasing N with P = 9 fixed, by sparsity level") + 
  theme_calc() 
```



### A bonus section: how to code a function that feels like `lm` in R
...
#' where the coefficients beta are obtained from the weighted least squares problem **min_b sum_i(w_i(y_i - x_i'b)^2)**
#' @params
#'   model: a character or formula input specifying a model, use y ~ x - 1 to remove the intercept
#'   data: a data.frame containing the variables of the formula, the data must have numeric columns only
#'   weights: a numeric vector of weights yo be assigned to each individual. Assigns equal weights by default.
#' @value a list with the model and numeric vector of cofficients of the weighted least squares regression
wls <- function(model, data, weights = rep(1, nrow(data)), method = c("qr", "inverse")) {
  # some input validation
  stopifnot(is.data.frame(data), is.numeric(weights))
  ...
  beta <- switch(
    method[1],
    "qr" = wls_qrfit(as.matrix(X), y, weights),
    "inverse" =  wls_invfit(as.matrix(X), y, weights)
  )
  names(beta) <- names(X)
  # output
  out <- list(
