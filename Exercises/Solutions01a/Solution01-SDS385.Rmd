...
```{r}
#' @title Weighted Least Squares Regression by QR Factorization
#' @description Uses QR factorization to solve fit the model
#'   y_i ~ x_i'b,
#' where the coefficients beta are obtained from the weighted least squares problem **min_b sum_i(w_i(y_i - x_i'b)^2)**
#' @details This function uses matrix computations to fit the WLS model. The design matrix X must be a formal object of the package Matrix; these guarantees appropriate and efficient internal algorithms. The QR factorization is apply directly to X so that the resulting system is $$RX = Q'y$$ with X and Y reweighted by rows/entries according to the square roots of the weights vector.
#' @params
#'   X the design matrix of predictor/independent variable. It must inherit from the class Matrix of the Matrix package: rows will correspond to individuals and columns to variables. 
...
wls_qrfit <- function(X, y, weights = rep(1, length(y))) {
  ...
  stopifnot(
    ...
    !anyNA(X) && !anyNA(y) && !anyNA(weights), # NAs not allowed
    nrow(X) == length(y) &&  nrow(X) == length(weights) # correct lengths
  )
  ...
  # 2. QR decomposition and system of equations
  qrdecomp <- qr(Xtilde, LAPACK = TRUE)
  # 3. Set up and solve the linear system
  beta <- qr.coef(qrdecomp, ytilde) # solves Rx = Q'y as in OLS
  as.numeric(beta)
}
```
...
```{r}
library(Matrix) # for the formal efficient Matrix class
library(dplyr) # for the pipe operator
data(iris)
X <- iris %>% 
  select(Petal.Length, Petal.Width) %>% 
  as("Matrix") # convert to efficient S4 object from Matrix class
y <- iris$Sepal.Width
...
```{r}
#' @title Weighted Least Squares Regression by Matrix Inversion
#' @description Uses QR factorization to solve fit the model
#'   y_i ~ x_i'b,
#' where the coefficients beta are obtained from the weighted least squares problem **min_b sum_i(w_i(y_i - x_i'b)^2)**
...
wls_invfit <- function(X, y, weights) {
  ...
  beta <- inverse %*% (crossprod(Xtilde, ytilde)) # system solution
  # output
  as.numeric(beta)
} 
```
