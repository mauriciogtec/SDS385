...
time_grid_N <- data.frame(
  Nvalue = floor(10^seq(2, 6, by = 0.5)) # 10, 31, 100, 316, etc..
)
...
*Note:* you could use the function `objetct.size` and vertify that the storage size of `X` greatly reduces once decleared as sparse
...
set.seed(110104) # reproducibility
Xdense <- rnorm(N * P) %>% 
  Matrix(ncol = P, sparse = TRUE) # We will mask Xdense later in our code with the different levels
sparsity <- c(.05, .25, .50, .75) # used to create many zeros later in X later in the code
# random true model
beta <- rnorm(P) # random coefficients
sigma <- .01 # error size
y <- drop(X %*% beta) + rnorm(N, sd = sigma)
...
```{r, eval = FALSE}`
Nvalue <- floor(10^seq(2, 6, by = 0.5)) # 100, 316, 1000, etc..
# create a grid of combinatios of sparsity and N
time_grid_N_sparse <- expand.grid(sparsity = sparsity, Nvalue = Nvalue)
time_grid_N_sparse$qr_time_sparse <- 0
time_grid_N_sparse$inv_time_sparse <- 0
for (i in seq_along(time_grid_N_sparse)) {
  n <- time_grid_N_sparse$Nvalue[i]
  sparsity <- time_grid_N_sparse$sparsity[i] 
  mask <- rbinom(n * P, 1, sparsity)
  X <- Xdense * mask # R knows this means multiply every entery
  t_qr <- microbenchmark(wls_qrfit(X[1:n, 1:P], y[1:n], weights[1:n]), times = 10L)
  t_inv <- microbenchmark(wls_invfit(X[1:n, 1:P], y[1:n], weights[1:n]), times = 10L)
...
plot_data <- time_grid_N_sparse %>% 
  mutate(logN = log(Nvalue, 10)) %>% 
  select(-Nvalue) %>% 
  gather(method, seconds, -c(logN, sparsity)) 
ggplot(plot_data, aes(x = logN, y = seconds, colour = method)) +
  facet_grid(sparsity ~ .) +
  geom_line(size = 1) +
  # scale_colour_discrete(labels = c("Full Inverse", "QR")) + 
