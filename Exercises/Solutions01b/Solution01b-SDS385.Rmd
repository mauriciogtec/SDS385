...
\end{aligned}
$$
We can further simplify this expression by completing the square in the quatratic expression above, yielding
$$
\begin{aligned}
l(\beta) & = \frac{1}{2}\left[\beta - \beta_0 + (X^\top\tilde{W} X)^{-1}X^\top(w(\beta_0) - y)\right]^\top X^\top\tilde{W} X \left[\beta - \beta_0 + (X^\top\tilde{W} X)^{-1}X^\top(w(\beta_0) - y)\right] + \text{const.} +  O\left(\lVert \beta - \beta_0 \rVert^3 \right) \\
\end{aligned}
$$
Denote $H = X(X^\top X)X^\top$ the linear projection matrix into the columns of $X$. Then the above has the form
$$
\begin{aligned}
l(\beta) & = \frac{1}{2}\left[X(\beta - \beta_0) + H(w(\beta_0) - y)\right]^\top \tilde{W} \left[X(\beta - \beta_0) + H (w(\beta_0) - y)\right] + \text{const.} +  O\left(\lVert \beta - \beta_0 \rVert^3 \right) \\
\end{aligned}
$$
