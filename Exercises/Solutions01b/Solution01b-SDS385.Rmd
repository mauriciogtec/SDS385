...
X <- cbind(1, scale(wdbc[ ,3:4])) # 1 adds an intercept
...
```{r}
logit_newthon_direction <- function(X, y, beta0) {
  ...
  prediction <- sigmoid(-reg) # Vector of current predictions
  error <- prediction - y # prediction error
  wlm_weights <- prediction * (1 - prediction)
  ...
  newthon_direction <- betanew - beta0
  # Output
  newthon_direction
}
```
...
```{r}
logit_newthon_fit <- function(X, y, beta0, 
                              history = TRUE, 
                              linesearch = TRUE, 
                              control = NULL, 
                              ls_control = NULL) {
  ...
  maxit <- 1e3
  tol <- 1e-11 # currently, only relative value convergence implemented |(fnew - fold) / fold| < rtol
  for (i in seq_along(names(control))) { 
    assign(names(control)[i], control[[i]]) # overwrite default
  }
  ...
  step_size_history <- 1
  repeat {
    newthon_direction <- logit_newthon_direction(X, y, beta)
    # Do linesearch if necessary
    if (linesearch) {
      btls <- bt_linesearch(
        x = beta, 
        search_direction = newthon_direction, 
        f = f, 
        grad = grad, 
        control = ls_control
      )
    ...
    beta <- beta + step_size * newthon_direction
    fnew <- f(beta)
    # Record values
...
```{r}
# Test
mod <- logit_newthon_fit(X, y, numeric(ncol(X)))
# print(mod)
```
