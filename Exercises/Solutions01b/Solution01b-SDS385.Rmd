...
\end{aligned}
$$
The expression is useful to compute $\nabla_\beta l(\beta)$, but later it will be useful to rewrite the above in a convenient way. Write $W = \textrm{diag}(w(\beta))$, then the *Jacobian* matrix $\nabla_\beta w(\beta) \in \mathbb{R}^{N \times P}$ of the map
$$
\begin{aligned}
\beta & \mapsto (w_1(\beta),...,w_N(\beta))
\end{aligned}
$$
is the whose $i$-th row is $\nabla_iw(\beta)$. We see that
$$
\nabla_\beta w(\beta) = X^\top W(1-W)
$$
