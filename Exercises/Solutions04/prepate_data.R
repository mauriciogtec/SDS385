# Author: James Scott 
# url: https://github.com/jgscott/SDS385/blob/master/R/read_svmlight_class.R
...
library(tidyverse)
library(Matrix)
X <- readRDS("Solutions04/Xtoy.RDS") 
y <- readRDS("Solutions04/ytoy.RDS")
m <- 1
lambda <- 0.1

res <- binom_fit(X[1:10000, ], Matrix(y[1:10000, drop = FALSE], sparse = TRUE), m, lambda, max_epochs = 1)
